...
do
    ...
    if [ ! -f "${tmpRunPath[$i]}/${tmpPName[$i]}" ]; then
        #echo "${tmpRunPath[$i]}/${tmpPName[$i]} is not exits!!!!!" >>${tmpAllLogDir}/"shStarPro"${tmpYMD}".log"
        continue
    fi
    ...
    if [ ${tmpPNum} -gt 0 ]; then
        ...
        echo "`date +%Y/%m/%d-%H:%M:%S.%N`: Program [${tmpRunPre[$i]} ${tmpRunPath[$i]}/${tmpPName[$i]} ${tmpRunPara[$i]}] start running" >>${tmpAllLogDir}/"shStarPro"${tmpYMD}".log"
    fi
done
