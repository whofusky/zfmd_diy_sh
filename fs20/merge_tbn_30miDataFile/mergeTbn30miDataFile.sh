...
curDoRltFileName=""        #当前程序需要生成的结果文件名（不带路径)
...
#
function F_judgeHaveDone() #return: 1 have done; 0 not have done
{
    ...
    local tnum
    tnum=$(egrep "^${curDoRltFileName}$" "${recordFile}"|wc -l)
    if [ ${tnum} -gt 0 ];then
        return 1
    fi
...
function F_genFileNames()
{
    ...
    if [ ${trueFlag} -eq 1 ];then  #当前小时的0-29分钟的文件
        ...
        curDoRltFileName="${g_upfile_frmName_domain}${g_upfile_joiner_char}${g_upfile_fanCode_domain}${g_upfile_joiner_char}${v_Year}${v_Month}${v_Day}${g_upfile_joiner_char}${v_Hour}00${g_upfile_suffix_domain}"
        ...
        curDoRltFileName="${g_upfile_frmName_domain}${g_upfile_joiner_char}${g_upfile_fanCode_domain}${g_upfile_joiner_char}${tYMD}${g_upfile_joiner_char}${tH}30${g_upfile_suffix_domain}"
        for ((i=0;i<30;i++))
        do
            ii=$(echo "${i} + 30"|bc)
...
{
    if [ $# -lt 2 ];then
        F_outShDebugMsg "${logFile}" 1 1 "ERROR:${FUNCNAME} input parameters less than 1!"
        return 1
    fi
    local tDir="$1"
    local tECNo="$2"
...
function F_doOneTurbByEcNo()  #根据风机的EC编号对某一个风机进行处理
{
    ...
    local doFiles="$3"
    local tDir="$4"
    local tmpFile="${tDir}/tmp_fj_EC_${tECNo}.txt"
    local tmpFileBak="${tDir}/tmp_fj_EC_${tECNo}.bak"
    ...
    if [ ${tnum} -eq 0 ];then
        F_outShDebugMsg "${logFile}" 1 1 "ERROR:${FUNCNAME}:[${curDoRltFileName}] ${doFiles} FJ EC=${tECNo} nums=${tnum}" 
        return 0
    fi
    ...
    if [ ${tnum} -lt ${need_perEC_items}  ];then
        F_outShDebugMsg "${logFile}" 1 1 "ERROR:${FUNCNAME}:[${curDoRltFileName}] ${doFiles} FJ EC=${tECNo} nums=${tnum},need_perEC_items=[${need_perEC_items}],g_supp_miss_item=[${g_supp_miss_item}]" 
        if [[ ! -z "${g_supp_miss_item}" && "${g_supp_miss_item}" = "1" ]];then
            F_fillMiss1miItem  "${tDir}" "${tECNo}"
        fi
    fi
...
function F_doTurb() #处理所有风机的数据
{
    ...
    F_outShDebugMsg "${logFile}" ${g_debugL_value} 1 "${FUNCNAME}:g_1mi_src_dir=[${g_1mi_src_dir}],g_dst_result_dir=[${g_dst_result_dir}]"
    F_outShDebugMsg "${logFile}" ${g_debugL_value} 1 "${FUNCNAME}:curDoFiles=[${curDoFiles}],curDoRltFileName=[${curDoRltFileName}]"
    ...
    local tRFileR="${g_dst_result_dir}/${curDoRltFileName}"
    local tRFile="${doSrcDir}/${curDoRltFileName}"
    local tnum=0
    ...
    echo "${resultHeadStr}">"${tRFile}"
    echo "${g_upfile_fixCnt_itemH}">>"${tRFile}"
    ...
    do
        ...
        F_outShDebugMsg "${logFile}" ${g_debugL_value} 16 "${FUNCNAME}:doing Ec[${tEcNo}]/${tMaxEc}----"

        F_doOneTurbByEcNo ${tEcNo} "${doSrcDir}" "${curDoFiles}" "${tmpDir}"
        tmpFile="${tmpDir}/tmp_fj_EC_${tEcNo}.txt"
        cat "${tmpFile}" >> "${tRFile}"
    done
    ...
    mv "${tRFile}"  "${g_dst_result_dir}"


    F_outShDebugMsg "${logFile}" ${g_debugL_value} 1 "${FUNCNAME}:[${g_dst_result_dir}/${curDoRltFileName}] do complete!"
