...
##############################################################################
# author  :  fusky
# date    :  2023-03-09_14:45:30
# dsc     :
#            根据gen_fs20_fs17_dir.sh生成的all_dir.txt及gen_busi_ftpdir.sh生成的genbusi_ftp_first_dir.txt
#             产生最终的:    all_dir.txt - genbusi_ftp_first_dir.txt
#
##############################################################################
...
function F_check()
{
    edFile1="${runDir}/all_dir.txt"                   #1. 用gen_fs20_fs17_dir.sh脚本根据bat文件生成的
    edFile2="${runDir}/genbusi_ftp_first_dir.txt"     #2. 用gen_busi_ftpdir.sh脚本根据业务清单生成配置生成的

    resultDir="${runDir}/result"
    [ ! -d "${resultDir}" ] && mkdir -p "${resultDir}"

    resultFile="${resultDir}/gen17.txt" # 上面 1 - 2 结果

    F_notFileExit "${edFile1}" "${edFile2}"
...
function F_gen17()
{
    F_writeLog "$DEBUG" "${LINENO}|${FUNCNAME}| edFile1=[${edFile1}]"
    F_writeLog "$DEBUG" "${LINENO}|${FUNCNAME}| edFile2=[${edFile2}]"
    sort "${edFile1}" "${edFile2}" "${edFile2}"|uniq -u >"${resultFile}"
