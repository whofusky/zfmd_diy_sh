...
##############################################################################
# author  :  fusky
# date    :  2023-03-09_10:02:06
# dsc     :
#            根据2.0业务清单合成的配置生成如下文件夹
#                 2.0合成业务清单需要的云平台目录 -  场内下载需要的文件夹
#
##############################################################################
...
function F_check()
{
    ...
    tempGenFile="${resultDir}/tmp_gen_dir.txt"          #1. 生成业务清单需要的云平台一级目录
    tempSiteDFile="${resultDir}/tmp_sitedown_dir.txt"   #2. 电场内下载业务清单需要的云平台一级目录
    resultFile="${resultDir}/genbusi_minus_onsitedown.txt" # 上面 1 - 2 结果
...
function F_genDirByCfgfile()
{
    ...
    sed -n '/^\s*<\s*downFile\s\+/p' "${edFile}"|awk '{for(i=1;i<=NF;i++){if($i ~/downDir=/){print $i;break;}}}'|awk -F'["/]' '{print $3}'|sort|uniq >"${tempGenFile}"
    sed -n '/^\s*<\s*upDir\s*>/p' "${edFile}"|awk -F'[<>]' '{print $3}'|awk -F'/' '{print $2}'|sort|uniq >"${tempSiteDFile}"
    sort "${tempGenFile}" "${tempSiteDFile}" "${tempSiteDFile}"|uniq -u >"${resultFile}"
