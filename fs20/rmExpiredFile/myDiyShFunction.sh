...
function F_rmExpiredFile() #call eg: F_rmExpiredFile "path" "days" OR F_rmExpiredFile "path" "days" "files"
{
    ...
    F_writeLog "$INFO" "${LINENO}|${FUNCNAME}|rm [${tpath}]'s [${tname}] , del num=[${tnum}]!"
...
function F_cfgFileCheck()
{
    ...
    local tdstNum=${#g_del_dir[*]}
    local tfilNum=${#g_file_name[*]}
    local tbscNum=${#g_expired_day[*]}

    if [[ ${tdstNum} -ne ${tfilNum} || ${tdstNum} -ne ${tbscNum} ]];then
        F_writeLog "$ERROR" "${LINENO}|${FUNCNAME}|cfgfile [${cfgFile}] 's set g_del_dir[x],g_file_name[x],g_expired_day[x] 's number not eq !\n"
